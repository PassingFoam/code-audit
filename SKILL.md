...
Step 7.2: 报告门禁验证 → 强制执行

> **⚠️ 门禁目的**: 确保最终报告完整且符合模板规范

**验证项 1: 漏洞完整性检查**
```
对比 Agent 临时报告与最终报告:
- Agent 临时报告总数 vs 最终报告漏洞数
- 缺失漏洞: 列出未被整合的漏洞 (agent_id + 漏洞标题)
- 缺失原因: 去重合并/遗漏/格式错误
```

**验证项 2: 模板符合性检查** (每条漏洞必填)

| 必填字段 | 验证规则 | 缺失处理 |
|---------|---------|---------|
| 严重程度 | Critical/High/Medium/Low | 标记 [需补充严重程度] |
| 漏洞标题 | 非空，描述漏洞类型 | 标记 [需补充标题] |
| 受影响组件 | 文件路径:行号 | 标记 [需补充位置] |
| 调用链 | `[入口点] → ... → [Sink]` 格式 | 标记 [需补充调用链] |
| 数据流路径 | `[Source] → ... → [Sink]` 格式 | 标记 [需补充数据流] |
| PoC | HTTP请求或代码payload | 标记 [需补充PoC] |
| 代码片段 | 来自 Read 工具输出 | 标记 [需补充代码片段] |

**验证项 3: 门禁结果输出**
```
[REPORT_GATE]
漏洞完整性: Agent 报告 {N} 条, 最终报告 {M} 条, 缺失 {N-M} 条
模板符合性: 完整 {X} 条, 缺失字段 {Y} 条
缺失详情: {列出缺失的漏洞和字段}

⚠️ 如有缺失:
- Critical/High 漏洞缺失 → 必须补充后才能输出报告
- Medium/Low 漏洞缺失 → 标记警告，可继续输出
```
