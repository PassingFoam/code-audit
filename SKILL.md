...
**Step 5.0: 设置执行检查点** (启动 Agent 前必须输出)

> **目的**: 防止上下文压缩导致步骤遗忘。检查点在最近上下文中，不会被压缩。
...
Agent IDs: {id_list}
完成后必须执行:
├── 每个 Agent 完成后 → Step 5.5 持久化门控
├── 所有 Agent 完成后 → Step 5.6 覆盖率验证
├── Step 5.6 完成后 → Step 6 报告门控
└── Step 6 通过后 → Step 7 报告保存
...
```
┌─────────────────────────────────────────────────────────────┐
│ Step 5.1: 启动 Multi-Agent (按 Step 4 确认的 Agent 方案)    │
└─────────────────────────────────────────────────────────────┘
    ↓
┌─────────────────────────────────────────────────────────────┐
│ Step 5.5: Agent 输出持久化门控                              │
│ 触发: 每个 Agent 完成后立即执行                             │
│ 确认: 必须输出 [AGENT_PERSISTED]                            │
│ 阻塞: 未确认前禁止启动下一个 Agent                          │
└─────────────────────────────────────────────────────────────┘
    ↓ (所有 Agent 完成后)
┌─────────────────────────────────────────────────────────────┐
│ Step 5.6: 调用点覆盖率验证                                  │
│ 触发: 所有 Agent 完成后执行一次                             │
│ 确认: 必须输出 [CALL_SITE_COVERAGE]                         │
│ 阻塞: 未确认前禁止进入 Step 6                               │
└─────────────────────────────────────────────────────────────┘
    ↓
┌─────────────────────────────────────────────────────────────┐
│ Step 6: 报告门控                                            │
│ 确认: 必须输出 [REPORT_GATE_PASSED] 或 [REPORT_GATE_FAILED] │
│ 阻塞: FAILED 时禁止进入 Step 7                              │
└─────────────────────────────────────────────────────────────┘
    ↓ (GATE PASSED)
┌─────────────────────────────────────────────────────────────┐
│ Step 7: 报告生成与保存                                      │
│ 确认: 必须输出 [REPORT_SAVED]                               │
└─────────────────────────────────────────────────────────────┘
```

**关键规则**:
1. 每个步骤必须输出确认标记才能进入下一步
2. 确认标记格式: `[STEP_NAME_DONE]` 或特定标记
3. 未输出确认标记 = 步骤未完成，禁止继续
4. **自检问题**: "我是否输出了确认标记？如果没有，我不能继续。"

### Step 5.5: Agent 输出持久化门控

> **⚠️ 强制门控**: 主线程收到 Agent 输出后，必须执行以下流程才能继续

**触发条件**: 收到 Agent 输出后**立即**执行（主线程阻塞式执行）
...
2. 【验证】检查写入内容:
   ...
   | 受影响组件 | 文件路径:行号 | [需补充位置] |
   | 攻击路径 | 包含[步骤N] + 代码 + 操作类型 + 入口点 + Sink | [需补充攻击路径] |
   | 攻击路径摘要 | 包含调用层级 + 数据跨度 + 净化检查 | [需补充路径摘要] |
...
4. 【确认输出】必须输出:
   ...
   ├── 持久化: ✓ 已写入临时文件
   ├── 验证: 攻击路径 {✓/⚠️} | 路径摘要 {✓/⚠️} | PoC {✓/⚠️} | 代码片段 {✓/⚠️}
   └── 压缩: ✓ 已释放上下文

5. 【更新检查点】更新 Step 5.0 的检查点状态:
   已返回 Agent: {n+1}/{total}
```

**阻塞条件**:
- 未输出 `[AGENT_PERSISTED]` → **禁止启动下一个 Agent**
- 未输出 `[AGENT_PERSISTED]` → **禁止进入 Step 5.6**

**自检问题**: "我是否输出了 `[AGENT_PERSISTED]`？如果没有，我不能继续。"

### Step 5.6: 调用点覆盖率验证

> **触发条件**: 所有 Agent 完成且都输出了 `[AGENT_PERSISTED]` 后执行

**前置检查**:
```
□ 所有 Agent 都输出了 `[AGENT_PERSISTED]`
□ Agent 数量 = Step 4 确认的 Agent 数量
□ 无 Agent 超时或已标注超时原因
```
...
**阻塞条件**:
- 未输出 `[CALL_SITE_COVERAGE]` → **禁止进入 Step 6**

**警告**: 覆盖率不足时，必须在报告中明确标注遗漏风险

**自检问题**: "我是否输出了 `[CALL_SITE_COVERAGE]`？如果没有，我不能进入报告门控。"
...
**阻塞条件**:
- 输出 `[REPORT_GATE_FAILED]` → **禁止进入 Step 7**
- 未输出确认标记 → **禁止进入 Step 7**

**自检问题**: "我是否输出了 `[REPORT_GATE_PASSED]`？如果没有，我不能生成报告。"
