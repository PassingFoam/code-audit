...
```
                    ┌─ 冗余路径: /etc/passwd/./././
                    ├─ 双斜杠: //etc//passwd
         规范化绕过─┼─ 反斜杠混用: ..\..\etc\passwd (Windows)
                    ├─ 空字节截断: ../../../etc/passwd%00.jpg (旧版本PHP/Java)
                    └─ URL编码: %2e%2e%2f (需服务端解码)
```
...
```
               ...
               过滤单个"..".──┤ 原理: "..../".replace("..", "") → "/"
                              │         所有".."被全局替换，无法形成新的".."
                              │
                              └─ 结论: 过滤".."在Java/Python中有效，但仍可能被编码绕过
               ...
               过滤"../" ─────┤ 类似绕过模式:
                              ...
                              │   ...././ → 删除位置2-4 → ./
                              │
                              └─ 结论: 过滤"../"可被绕过! 必须使用路径规范化验证

                              ┌─ 绝对路径: /etc/passwd (不使用../)
         其他绕过 ─────────────┼─ 符号链接: 上传包含符号链接的压缩包
                              └─ 文件协议: file:///etc/passwd
```

**语言无关的绕过规律**：

| 过滤目标 | 输入 | Java/PHP/Python替换后结果 | 可绕过? |
|---------|------|-------------------------|--------|
| `".."` | `..../` | `/` | ❌ 无法绕过 (全局替换) |
| `".."` | `....//` | `//` | ❌ 无法绕过 (全局替换) |
| `"../"` | `....//` | `../` | ✅ **可绕过** |
...
**核心洞察**：过滤 `../`（完整单元）时，删除匹配项后剩余字符可能重新组成有效的路径穿越序列！

**安全实践**：
```java
// ❌ 错误: 过滤"../"可被绕过
path = path.replace("../", "");  // ....// 替换后形成 ../

// ⚠️ 警告: 过滤".."虽然有效，但仍不推荐
path = path.replace("..", "");   // 可被编码绕过 (%2e%2e)

// ✅ 正确: 路径规范化 + 边界验证
Path basePath = Paths.get("/safe/dir").normalize().toAbsolutePath();
Path resolvedPath = basePath.resolve(userInput).normalize().toAbsolutePath();
