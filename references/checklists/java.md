...
- 在后缀名被限制的情况，只要可以路径穿越，依然是一个严重的问题，例如传到`/etc/bash_completion.d/` 文件夹下
- 过滤 `../` 可被绕过：`....//` 过滤后变为 `../`（删除中间匹配后剩余字符形成新序列）
- 过滤 `..` 在Java中无法绕过：`..../`.replace("..","") → `/`（全局替换）
- 编码绕过需二次解码：`%2e%2e` 只有在服务端多次解码时才有效（如Web容器解码+应用层解码）
