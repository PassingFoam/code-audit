...
5. **★ 漏洞即时持久化**: 每发现一个漏洞，立即使用 Write 工具写入临时报告文件
   - 路径: `.audit-reports/.tmp/agent_{维度}_{序号}.md`
   - 格式: 使用「漏洞报告模板 (标准版)」
   - 输出前验证: 调用链、数据流路径、PoC、代码片段四项检查
   - 上下文保留: 仅保留漏洞摘要表，详细分析写入文件后删除
...
### 报告输出强制检查 (Report Gate)

在输出最终报告前，对每个漏洞执行以下检查：
...
**质量门控**: 任一必填项缺失 → 该漏洞不得标记为 `[已验证]`，最高置信度为 `[高置信]`
